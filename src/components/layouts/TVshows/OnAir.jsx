...
export default function OnAir() {
  ...
  const { isLoading, data: movies } = useFetch(
    `https://api.themoviedb.org/3/tv/on_the_air?api_key=${VITE_API_KEY}&language=en-US&page=${currentPage}`,
    "getTVOnAirLayout",
    "results"
  );
  ...
  const onAirArr = movies?.map((movie) => {
    const releaseDate =
  ...
  return (
    <>
      <div className="genreMovie">
        {isLoading ? <Loader isLoading={isLoading} /> : onAirArr}
      </div>
