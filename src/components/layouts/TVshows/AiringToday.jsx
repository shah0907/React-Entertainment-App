...
export default function AiringToday() {
  ...
  const { isLoading, data: movies } = useFetch(
    `https://api.themoviedb.org/3/tv/airing_today?api_key=${VITE_API_KEY}&language=en-US&page=${currentPage}`,
    "getAiringTodayLayout",
    "results"
  );

  const { data: totalNumberOfPages } = useFetch(
    `https://api.themoviedb.org/3/tv/airing_today?api_key=${VITE_API_KEY}&language=en-US&page=${currentPage}`,
    "totalNumberOfPagesAiringToday",
    "total_pages"
  );
  ...
  const airingTodayArr = movies?.map((movie) => {
    const releaseDate =
  ...
  return (
    <>
      <div className="genreMovie">
        {isLoading ? <Loader isLoading={isLoading} /> : airingTodayArr}
      </div>
